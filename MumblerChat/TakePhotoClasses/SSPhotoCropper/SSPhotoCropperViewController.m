...
//    float imageH = self.photo.size.height;
//    float hScaleFactor = viewH/imageH;
    
//    float viewW = self.view.frame.size.width;
//    float imageW = self.photo.size.width;
//    float WScaleFactor = viewW/imageW;
    
   /* // resize image
    UIGraphicsBeginImageContextWithOptions(self.photo.size, NO, 0.0);
    [self.photo drawInRect:CGRectMake(0, 0, (self.photo.size.width * WScaleFactor), (self.photo.size.height * hScaleFactor))];
    ...
    CGRect imageViewFrame = CGRectMake(0.0f, 0.0f, roundf(w ), roundf(h));
...
- (UIImage *) croppedPhoto
{
   /* CGFloat ox = self.scrollView.contentOffset.x;
    ...
    // ruchira get crop rect
    CGFloat ox = self.scrollView.contentOffset.x;
    CGFloat oy = self.scrollView.contentOffset.y;
    CGFloat zoomScale = self.scrollView.zoomScale;
    CGFloat cx = (ox + self.cropRectangleButton.frame.origin.x)/zoomScale;
    CGFloat cy = (oy + self.cropRectangleButton.frame.origin.y) / zoomScale;
    CGFloat cw =  self.cropRectangleButton.frame.size.width / zoomScale;
    CGFloat ch =  self.cropRectangleButton.frame.size.height / zoomScale;
